...
# - Riesgo de cohorte para clientes nuevos (opcional pseudo-label)
...
try:
    ...
    _HAS_LGB = False
    warnings.warn("⚠ No se pudo importar LightGBM. Se usará RandomForest como fallback.")

# ----------------------- RUTAS -------------------------------
DATA_PARQUET_PATH = "data/dataset_ml_final.parquet"
TRANSFORMER_PATH = "data/transformer.pkl"   # opcional: si quieres transformar aquí
MODELS_DIR = "models"
REPORTS_DIR = "reports"
FILES_DIR = "files"  # Para reportes consolidados
...
def ks_statistic(y_true, y_score):
    data = pd.DataFrame({"y": y_true, "p": y_score}).sort_values("p", ascending=True)
    pos = (data["y"] == 1).sum()
    neg = (data["y"] == 0).sum()
    if pos == 0 or neg == 0:
        return 0.0
...
#  CARGA Y PREP DE DATOS 
def load_base_dataset(path=DATA_PARQUET_PATH) -> pd.DataFrame:
    if not os.path.exists(path):
        raise FileNotFoundError(f"No se encontró dataset base en {path}. Ejecuta primero data_prep.py")
    df = pd.read_parquet(path)
    return df

def apply_rolling_window(df: pd.DataFrame, months:int) -> pd.DataFrame:
    """
    Filtra por ventana móvil usando la fecha de colocación del contrato (registro_contrato si existe).
    """
    if "registro_contrato" in df.columns:
        ...
        mask = (fecha_col.dt.tz_localize(None) >= cutoff) if hasattr(fecha_col.dtype, "tz") else (fecha_col >= cutoff)
        return df.loc[mask].copy()
    return df.copy()

def compute_time_weights(df: pd.DataFrame, alpha:float=0.05) -> np.ndarray:
    """
    Ponderación temporal exponencial: registros más recientes pesan más.
    weight = exp(-alpha * months_ago)
    """
...
def add_target_and_cohorts(df: pd.DataFrame, dpd_threshold:int=0, include_pseudolabels:bool=False) -> Bunch:
    """
    Crea columna target para EXISTENTES y calcula riesgo de cohorte para NUEVOS.
    Label por negocio: 1 si dpd_max_6m > dpd_threshold (default 0).
    """
    df = df.copy()

    # Asegurar tipo_cliente (debería venir desde data_prep)
    ...
    # Base para cálculo de tasas de cohorte (solo existentes con target ya observado)
    ...
    # ⚠️ Corrección: usar agente_acreditario (existe en tu dataset)
    ...
    def riesgo_fila(row):
        vals = []
        if "tipo_contrato" in row and row["tipo_contrato"] in c_prod:
            vals.append(c_prod[row["tipo_contrato"]])
        if "agente_acreditario" in row and row["agente_acreditario"] in c_agente:
            vals.append(c_agente[row["agente_acreditario"]])
        if "mes_colocacion" in row and row["mes_colocacion"] in c_mes:
    ...
    if include_pseudolabels and mask_nuevos.any():
        thr = float(base_rate)
        df.loc[mask_nuevos, "target"] = (df.loc[mask_nuevos, "riesgo_cohorte"].fillna(base_rate) >= thr).astype(int)

    return Bunch(df=df, base_rate=float(base_rate))

# Selección/Construcción de X (fallback sin transformer)
LOW_CARD_CATS = [
    "estado_de_prestamo", "tiempo_de_pago", "tipo_contrato",
    "frecuencia_pago", "agente_acreditario", "vehiculo_combustible", "vehiculo_clase"
]

ID_LIKE = {
    "id","codigo_contrato","id_prestante","id_registrador","id_aval","id_conyugue",
    "numero_dni","nombre_apellido_prestante","imagenes_producto","datos_bien",
    "created","updated","registro_contrato","collection_id","collection_name","ubicacion_producto"
}

LEAKY = {
    "dpd_max_6m","dpd_prom_6m","pagos_esperados_6m","pagos_realizados_6m",
    "ratio_pago_cuotas_6m","dias_desde_ultimo_pago_a_tiempo","target","riesgo_cohorte"
}

def build_X_y(df: pd.DataFrame):
    """
    Crea X (features) y y (target).
    - Si existe transformer.pkl → lo usa para garantizar consistencia con producción.
    - Si no existe → fallback: numéricas + dummies de categóricas de baja cardinalidad.
    - EXCLUYE columnas con fuga (LEAKY) e identificadores (ID_LIKE).
    """
    ...
    if os.path.exists(TRANSFORMER_PATH):
        ...
        feature_cols = getattr(transformer, "get_feature_names_out", lambda: [])()
    ...
    # Limitar categóricas a las conocidas de baja cardinalidad (intersección)
    cat_cols = [c for c in cat_cols if c in LOW_CARD_CATS and c in df.columns]

    base = df[num_cols + cat_cols].copy()
    # Imputo nulos básicos
    for c in num_cols:
...
def train_full(df: pd.DataFrame, args):
    """
    Reentrenamiento completo (batch). Aplica rolling window y weights temporales.
    """
    if args.rolling_months > 0:
        df = apply_rolling_window(df, args.rolling_months)
        print(f"🔁 Rolling window aplicada: últimos {args.rolling_months} meses → {len(df)} filas")

    # Target + cohortes
    b = add_target_and_cohorts(df, dpd_threshold=args.dpd_threshold, include_pseudolabels=args.include_pseudolabels)
    df = b.df

    # Filtrar filas con target
    ...
    # Train/test split estratificado si es posible
    strat = y if len(np.unique(y)) > 1 else None
    X_tr, X_te, y_tr, y_te, w_tr, w_te = train_test_split(
        X, y, time_w, test_size=0.2, random_state=42, stratify=strat
    )
    ...
    # Guardar modelo y registrar en versioning
    model_path = os.path.join(MODELS_DIR, f"model_v{vers}.pkl")
    joblib.dump(model, model_path)
    versioning.register_model(vers, model_path, status="INACTIVO")  # REGISTRO
    versioning.activate_model(vers)  # ✅ ACTIVAR AUTOMÁTICAMENTE
    ...
    # Guardar metadata de features del fallback (si aplica)
    ...
    print(f"✅ Modelo guardado y ACTIVADO: {model_path}")
    print(f"📊 Métricas: AUC={metrics['auc']}, KS={metrics['ks']}, F1={metrics['f1']}, Precision={metrics['precision']}, Recall={metrics['recall']}")

    # Actualizar métricas históricas y PDF consolidado
    print("📊 Actualizando histórico de métricas y reporte consolidado...")
    os.system("python src/ml/metrics.py --update-history")
    os.system("python src/ml/metrics.py --generate-pdf")
    print("✅ Reportes históricos actualizados en /files/")

    return model_path, vers

def train_incremental(df: pd.DataFrame, args):
    """
    Entrenamiento incremental: continúa desde el último modelo si existe.
    Si no existe, cae en FULL train.
    """
    b = add_target_and_cohorts(df, dpd_threshold=args.dpd_threshold, include_pseudolabels=args.include_pseudolabels)
    df = b.df
    df = df[df["target"].isin([0, 1])].copy()
    ...
    if not _HAS_LGB:
        warnings.warn("⚠ LightGBM no disponible; usando FULL train porque incremental requiere booster.")
        return train_full(df, args)

    # Buscar último modelo para continuar entrenamiento
    last_version = next_model_version() - 1
    last_model_path = os.path.join(MODELS_DIR, f"model_v{last_version}.pkl")
    if not os.path.exists(last_model_path):
        print("ℹ️ No se encontró modelo previo. Ejecutando FULL train.")
        return train_full(df, args)

    init_model = joblib.load(last_model_path)

    booster = None
    if hasattr(init_model, "booster_"):  # LGBMClassifier estilo sklearn
        booster = init_model.booster_
    elif isinstance(init_model, LGBMClassifier.Booster):
        booster = init_model

    if booster is None:
        print("⚠ No se pudo reutilizar booster. Ejecutando FULL train.")
        return train_full(df, args)
    ...
    print(f"✅ Entrenamiento incremental completado → model_v{vers}.pkl ACTIVADO")
    os.system("python src/ml/metrics.py --update-history")
    os.system("python src/ml/metrics.py --generate-pdf")
    print("✅ Reportes históricos actualizados en /files/")
...
def main():
    ...
    parser.add_argument("--rolling_months", type=int, default=12, help="Ventana móvil en meses para FULL retrain.")
    parser.add_argument("--dpd_threshold", type=int, default=0, help="Umbral de mora (días) para target=1. (Default=0)")
    parser.add_argument("--time_decay_alpha", type=float, default=0.05, help="Decaimiento temporal para ponderación.")
    ...
    parser.add_argument("--include_pseudolabels", action="store_true",
                        help="Si se activa, nuevos clientes usan pseudo-label a partir de riesgo de cohorte.")
    args = parser.parse_args()

    print(f"⚙️ Modo: {args.mode} | rolling={args.rolling_months}m | dpd>{args.dpd_threshold} | alpha={args.time_decay_alpha}")
